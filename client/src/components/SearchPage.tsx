        ...
        <div className="flex-1">
          <div className="flex items-start gap-2">
            <Input
              ...
              className="shadow-lg focus-visible:ring-0"
            />
              ...
              <div className="flex flex-wrap gap-2 mb-4 md:mb-0">
                {
                  appliedFilter.map((selectedFilter: string, idx: number) => (
                    <div key={idx} className="relative inline-flex items-center max-w-full ">
                      <Badge className="text-[#D19254] rounded-md cursor-pointer pr-6 whitespace-nowrap" variant="outline" >{selectedFilter}</Badge>
                      <X
                      onClick={()=>setAppliedFilter(selectedFilter)}
                      size={16} className="absolute text-[#D19254] right-2 hover:cursor-pointer" />
                    </div>
                  ))
                }
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {
                  ...
                  searchedRestaurant?.data.map((item: Restaurant ) => (
                    <Card key={item._id} className="bg-white dark:bg-gray-800 shadow-xl rounded-xl overflow-hidden hover:shadow-2xl transition-shadow duration-300">
                      ...
                      <CardFooter className="p-4 border-t dark:border-t-gray-700 border-t-gray-100 text-white flex justify-end">
                        <Link to={`/restaurant/${item._id}`}>
                          <Button className="bg-orange hover:bg-hoverOrange font-semibold py-2 px-4 rounded-full shadow-xl transition-colors duration-200">View Menus</Button></Link>
                      </CardFooter>
                    </Card>
                  ))
                    )
